...
export default {
  data() {
    return {
      ...
      format: "doc/xls/ppt/txt/docx/xlsx/pptx/pdf/rar/zip",
  ...
  methods: {
    saveMedia: async function(params) {
      this.uploadingStatusMixin = true // 开始上传
      /**
       * params = {
      ...
      if (response) {
        ...
        this.uploadingStatusMixin = false // 上传结束
    ...
    async notFile(param) {
      ...
      if (param.metarialType === "video") { // 视频媒体必须传 desc
    ...
    autoSaveMedia: async function (param) {
      // session, add type  node 3 image, audio, video, file
      const params = {
    ...
    async isFile(param) {
      ...
      let params = {
        ...
        name: param.title || this.file.name,
        size: this.file.size,
        appAccountId: param.appAccountId,
        groupId: param.groupId,
        type: param.fileSourceType, // 1：本地文件 ， 2：资源文件
        url: param.response.split('cloud.com')[1]
      }
      ...
      if (res.data.code === 1) {
        this.$Message.success("保存成功！")
        this.file = {}
        this.afterSaveMedia && this.afterSaveMedia({ response: param.response, ...res.data, ...params })
      } else {
        this.uploadingStatusMixin = false
        this.$Message.error(res.data.message)
      }

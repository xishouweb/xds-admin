<template>
  <div class="label-list" :style="{width}" >
    ...
    :style="{width: parseInt(width) + 30 + 'px', paddingRight: '30px'}"
      placeholder :data="groupList">
      <div class="groupList">
        <ul>
          <li
            ...
            <template v-show="item.count">
              <span class="label-count" v-if="index === 0">({{totalCount || item.count}})</span>
              <span class="label-count" v-else-if="item.count">({{item.count}})</span>
            </template>
            <div
              class="action"
              @click.stop="operation($event, item, index)"
              v-if="index >= ableActionIndex  && type === 'wx'">
    ...
    <Modal title="删除提示" v-model="showModal" class-name="self-center-modal">
      <div>分组删除后，分组下的内容将被放在未分组下，确定要删除吗？</div>
      <div slot="footer">
  ...
  methods: {
    ...
    async modalConfirm() {
      if (!this.currPublic) return
        ...
        let res = await this.$post(this.Path.deleteGroup, param)

...
<div class="material-picture-container">
  <Frame
      ...
      <template slot="header">
        <HeaderAction placeholder="输入视频名称搜索"
        :info="numberInfo"
         ...
         label3="上传视频"></HeaderAction>
      </template>

      <template slot="label">
        <LabelList :labelList="labelList" :type='sourceType' @createNew="createNewLabel" @selectGroup="selectLabel" @deleteGroup="init"></LabelList>
      </template>
      ...
      <template slot="main">
        ...
        <Loading v-show="!linkList.groupList.length" :loading="loading">暂无视频</Loading>
        <div v-show="linkList.groupList.length" >
          <YktTable v-if="sourceType == 'wx'" @getcurrent="tableAction" :datas="linkList.groupList" :selectType="linkList.type" :type="metarialType" size="large" gutter="10" width="230" height="143"></YktTable>
          <CloudTable v-else @on-change="getSelectedLinkListItem" @getcurrent="tableAction" :datas="linkList.groupList" :selectType="linkList.type" :type="metarialType" size="large" gutter="10" width="230" height="143"></CloudTable>
        </div>
      </template>
  </Frame>
  ...
  <Drawer v-model="drawer.show" :titleTips="drawer.title" :isScroll="false" width ="530px"  ref="Drawer">
    <div slot="content">
      <DrawerContent :drawer="drawer" :labelList="labelList" :metarialType="metarialType" v-if="drawer.show"
          ...
          @uploadSuccess="uploadSuccess"></DrawerContent>
    </div>
  </Drawer>
  ...
  methods: {
    async init() {
        ...
        if (this.sourceType === 'wx') {
          ...
          this.labelList = this.deepCopy(this.getVideoGroupList)
    ...
    async getGroupList(first) {
      ...
      if (this.sourceType === 'wx') {
        ...
        this.queryTmpMediaVideoList(param).then(res => {
          if (res.code === 1) {
            if (!first) {
              this.loading = false
              this.listParam = Object.assign({}, this.listParam, {
